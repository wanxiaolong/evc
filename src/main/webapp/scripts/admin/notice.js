...
$(document).ready(function(){
	...
	//表格操作列上的“编辑”按钮的点击事件
	$('#noticeTable tbody').on('click', 'button#editrow', function() {
		//先将行的数据转换成数组
	...
	//表格操作列上的“删除”按钮的点击事件
	$('#noticeTable tbody').on('click', 'button#deleterow', function() {
		//先将行的数据转换成数组
		var data = table.row($(this).parents('tr')).data();
		//第一列（页面上不显示）里保存的是完整的数据对象
		del(data[0]);
...
//点击“删除”按钮后的操作
function del(notice) {
	ajax('POST', '/notice/delete', 'id=' + notice.id, null, deleteSuccessCallback);
}
...
function edit(notice) {
	var fields = $("#edit-form").serializeArray();
	$.each(fields, function(i, field){
		if(field.name == 'importantLevel') {
			var importantLevelStr = convertImportantLevelToStr(notice[field.name]);
			$(":input[name='"+field.name+"']").val(importantLevelStr);
		}
		
		//jquery根据name属性查找input，然后从score中获取对应的属性值，并将值设置给input
		//注意这里只能通过[]的方式来获取属性值
		$(":input[name='"+field.name+"']").val(notice[field.name]);
	});
}
...
function submitUpdate() {
	...
	var importantLevel = $("#edit-form input[name='importantLevel']").val();
	var content = $("#edit-form input[name='content']").val();
	
	var isCreate = id == '';
	var url = isCreate ? '/notice/create' : '/notice/update';
	var data =	'id=' + id +
				'&title=' + title +
				'&important_level=' + convertImportantLevelToInt(importantLevel) +
				'&content=' + content;
...
//转换重要级别。把0=普通，1=一般，2=高
function convertImportantLevelToStr(level) {
	if (level == 0) return '默认';
	if (level == 1) return '一般';
	if (level == 2) return '高';
}
//转换重要级别。把0=普通，1=一般，2=高
function convertImportantLevelToInt(level) {
	if (level == '默认') return 0;
	if (level == '一般') return 1;
	if (level == '高') return 2;
}