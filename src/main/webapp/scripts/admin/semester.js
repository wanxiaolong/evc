/**
 * 本文件为admin/semester.jsp使用的初始化脚本。
 */
var allSubjects;//保存所有的科目信息
var webroot = getWebRoot();
$(document).ready(function(){
	table = initDataTable("semesterTable");
	
	//表格操作列上的“编辑”按钮的点击事件
	$('#semesterTable tbody').on('click', 'button#editrow', function() {
		//先将行的数据转换成数组
	...
	//表格操作列上的“删除”按钮的点击事件
	$('#semesterTable tbody').on('click', 'button#deleterow', function() {
		//先将行的数据转换成数组
		var data = table.row($(this).parents('tr')).data();
		//第一列（页面上不显示）里保存的是完整的数据对象
		del(data[0]);
...
//查询所有的学期信息
function queryAll() {
	ajax('GET', '/semester/all', null, null, addRows);
}
...
//点击“删除”按钮后的操作
function del(semester) {
	ajax('POST', '/semester/delete', 'id=' + semester.id, null, deleteSuccessCallback);
}
...
function submitUpdate() {
	...
	var isCreate = id == '';
	var url = isCreate ? '/semester/create' : '/semester/update';
	var data =	'id=' + id +
				'&name=' + name;
